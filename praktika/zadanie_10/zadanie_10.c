/*
 * Задание 10 — 7SEG PORTB, числа 6..36. 16 МГц. Atmel Studio.
 */
...
void SSD_Show(unsigned char number)
{
    ...
    PORTB = seg_table[tens];   /* код десятков на сегменты PORTB */
    ...
    PORTB = seg_table[ones];   /* код единиц на сегменты PORTB */
    PORTC |= (1 << PC0); /* отключить разряд десятков */
...
int main(void)
{
    ...
    value = 6;                    /* начальное значение */
    DDRB = 0xFF;               /* PORTB — все выходы (сегменты) */
